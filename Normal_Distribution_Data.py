...
def gammadef(z):
    return quad(integrand, 0, np.inf, args = z)[0]

def integrand(t,z):
    return (math.e**(-t)) * (t**(z-1))
...
def create_training_set(num_samples, num_x_vals = 21): #pass in number of x values as parameter instead of hard coding, accuracy varies with input?
    ...
    for i in range(num_samples):
        ...
        random_beta = uniform(1, 5) #(0,1) is in the domain too but change later to accomodate for complex
